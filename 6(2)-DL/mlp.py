...
class MultiLayerPerceptron(object):
    ...
    def back_propagation(self, cache, X, y, L2_norm=0.0):
        ...
        dy_hat = (y_hat - y) / (y_hat * (1 - y_hat) + 1e-3)
        dh4 = dy_hat * y_hat * (1 - y_hat)
        db4 = np.sum(dh4, axis=0)
        dW4 = z3.T @ dh4 + 2 * L2_norm * W4
        dz3 = dh4 @ W4.T

        # Layer 3 gradients
        dh3 = dz3 * (1 - np.tanh(h3 + 0.1 * h1)**2)  # Adjusted for skip connection
    ...
    def train(self, X_train, y_train, X_val=None, y_val=None, learning_rate=1e-3, L2_norm=0.0, epoch=20000, print_loss=True):
        ...
        Args:
            ...
            y_val: (numpy array) validation labels (N,)
        ...
        for it in range(epoch):
            ...
            y_hat, cache = self.forward_propagation(X_train)  # Forward propagation
            loss = self.compute_loss(y_hat, y_train, L2_norm)  # Compute loss
            
            # 2. Backward propagation
            grads = self.back_propagation(cache, X_train, y_train, L2_norm)  # Gradients
